...
public final class AESGCMCipher extends CipherSpi implements AESConstants, GCMConstants {
        ...
        try {
            if (encrypting) {
                ...
                if ((!sbeInLastFinalEncrypt) && encrypting && !initCalledInEncSeq) {
                    boolean sameKeyIv = checkKeyAndNonce(Key.get(), IV, lastEncKey, lastEncIv);
                    if (sameKeyIv) {
                        resetVars(true);
                ...
                int ret = GCMCipher.doGCMFinal_Encrypt(ockContext, Key.get(), IV, tagLenInBytes, input,
                ...
                int ret = GCMCipher.doGCMFinal_Decrypt(ockContext, Key.get(), IV, tagLenInBytes, input,
                        inputOffset, inputLen, output, outputOffset, authData, provider);
                authData = null; // Before returning from doFinal(), restore AAD to uninitialized state
                return ret;
            }
    ...
    private void internalInit(int opmode, Key key, byte[] iv) throws InvalidKeyException {
        ...
        try {
            ...
            this.newIV = null;
            this.Key = new ByteWrapper(rawKey.clone());
            this.IV = iv.clone();
        ...
        try {
            ...
            if (firstUpdate) {
                if (!encrypting) {
                    ...
                    outLen = GCMCipher.do_GCM_InitForUpdateDecrypt(ockContext, Key.get(), IV,
                    ...
                    outLen = GCMCipher.do_GCM_InitForUpdateEncrypt(ockContext, Key.get(), IV,
                            tagLenInBytes, buffer, 0, len, output, outputOffset, authData, provider);
                    // OCKDebug.Msg(debPrefix, methodName, "returning ret from
            ...
            if (len > 0) {
                ...
                if (len <= buffered) {
                    ...
                    if (!encrypting) {
                        ...
                        outLen = GCMCipher.do_GCM_UpdForUpdateDecrypt(ockContext, Key.get(), IV,
                                tagLenInBytes, buffer, 0, len, output, outputOffset, authData, provider);
                        ...
                        outLen = GCMCipher.do_GCM_UpdForUpdateEncrypt(ockContext, Key.get(), IV,
                                tagLenInBytes, buffer, 0, len, output, outputOffset, authData, provider);
                        // OCKDebug.Msg(debPrefix, methodName, "returning ret from
    ...
    private int finalNoPadding(byte[] in, int inOfs, byte[] out, int outOfs, int len)
        ...
        if (!encrypting) {
            outLen = GCMCipher.do_GCM_FinalForUpdateDecrypt(ockContext, Key.get(), IV, tagLenInBytes, in,
            ...
            outLen = GCMCipher.do_GCM_FinalForUpdateEncrypt(ockContext, Key.get(), IV, tagLenInBytes, in,
                    inOfs, len, out, outOfs, authData, provider);
            // OCKDebug.Msg(debPrefix, methodName, "outLen from
    ...
    private Runnable cleanOCKResources(ByteWrapper Key) {
        return() -> {
            try {
